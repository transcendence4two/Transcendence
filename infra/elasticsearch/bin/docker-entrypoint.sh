...
# Start Elasticsearch in the background, then configure kibana_system password.
/usr/share/elasticsearch/bin/elasticsearch "$@" $POSITIONAL_PARAMETERS <<<"$KEYSTORE_PASSWORD" &
es_pid=$!
...
if [[ -n "$ELASTIC_PASSWORD" && -n "$ELASTICSEARCH_PASSWORD" ]]; then
  echo "Waiting Elasticsearch..."
  for i in {1..60}; do
    if curl -fsS -u "elastic:${ELASTIC_PASSWORD}" http://localhost:9200/_cluster/health | grep -q 'status'; then
      break
    fi
    sleep 5
  done
  ...
  if ! curl -fsS -X POST \
       ...
       -d "{\"password\":\"${ELASTICSEARCH_PASSWORD}\"}" >/dev/null; then
    echo "ERROR: failed to set kibana_system password" >&2
    exit 1
  fi
fi
